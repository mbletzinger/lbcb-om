...
extern "C" {
_declspec(dllexport) void LBCB_conversion_C2A(long size, long type, double motion_center[], double sensor_reading[], double cartesian_value[]);
}
...
_declspec(dllexport) void LBCB_conversion_C2A(long size, long type, double motion_center[], double sensor_reading[], double cartesian_value[])
	...
	VECTOR ActuatorSpace(6), CartesianData(6);
	VECTOR MotionCenterData(6);
	VECTOR MotionCenter(3), PlatformCenter(3);
	...
	RigidTransform( MotionCenter, MotionCenterData, PlatformCenter, CartesianData );
