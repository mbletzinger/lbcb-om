...
{
	LBCB::SetErrorLogger(&elog);
	LBCB_Actuator::SetErrorLogger(&elog);
	VECTOR::SetErrorLogger(&elog);
	MATRIX::SetErrorLogger(&elog);
	RotationalMatrix::SetErrorLogger(&elog);
}
_declspec(dllexport) void SetErrorLogFile(char* filename, long length)
{
	string Filename(filename,filename + length - 1);
	elog.setFile(Filename);
}